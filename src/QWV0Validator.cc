...
class QWV0Validator : public edm::EDAnalyzer {
 ...
 private:
  ...
  void bookHistograms(edm::Service<TFileService> &, edm::Run const &,
  ...
  std::array<TH1D *, 2> candidateEffVsR_num_;
  std::array<TH1D *, 2> candidateEffVsEta_num_;
  std::array<TH1D *, 2> candidateEffVsPt_num_;
  std::array<TH1D *, 2> candidateTkEffVsR_num_;
  std::array<TH1D *, 2> candidateTkEffVsEta_num_;
  std::array<TH1D *, 2> candidateTkEffVsPt_num_;
  std::array<TH1D *, 2> candidateFakeVsR_num_;
  std::array<TH1D *, 2> candidateFakeVsEta_num_;
  std::array<TH1D *, 2> candidateFakeVsPt_num_;
  std::array<TH1D *, 2> candidateTkFakeVsR_num_;
  std::array<TH1D *, 2> candidateTkFakeVsEta_num_;
  std::array<TH1D *, 2> candidateTkFakeVsPt_num_;

  std::array<TH1D *, 2> candidateFakeVsR_denom_;
  std::array<TH1D *, 2> candidateFakeVsEta_denom_;
  std::array<TH1D *, 2> candidateFakeVsPt_denom_;
  std::array<TH1D *, 2> candidateEffVsR_denom_;
  std::array<TH1D *, 2> candidateEffVsEta_denom_;
  std::array<TH1D *, 2> candidateEffVsPt_denom_;
...
void QWV0Validator::bookHistograms(edm::Service<TFileService> & fs, edm::Run const&,
                                 edm::EventSetup const&) {
  ...
  candidateEffVsR_num_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sEffVsR_num", "K^{0}_{S} Efficiency vs #rho", 80, 0., 40.);
  candidateEffVsEta_num_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sEffVsEta_num", "K^{0}_{S} Efficiency vs #eta", 40, -2.5, 2.5);
  candidateEffVsPt_num_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sEffVsPt_num", "K^{0}_{S} Efficiency vs p_{T}", 70, 0., 20.);

  candidateTkEffVsR_num_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sTkEffVsR_num", "K^{0}_{S} Tracking Efficiency vs #rho", 80, 0., 40.);
  candidateTkEffVsEta_num_[QWV0Validator::KSHORT] =
      fKs.make<TH1D>("K0sTkEffVsEta_num",
                     "K^{0}_{S} Tracking Efficiency vs #eta", 40, -2.5, 2.5);
  candidateTkEffVsPt_num_[QWV0Validator::KSHORT] =
      fKs.make<TH1D>("K0sTkEffVsPt_num",
                     "K^{0}_{S} Tracking Efficiency vs p_{T}", 70, 0., 20.);

  candidateEffVsR_denom_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sEffVsR_denom", "K^{0}_{S} Efficiency vs #rho", 80, 0., 40.);
  candidateEffVsEta_denom_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sEffVsEta_denom", "K^{0}_{S} Efficiency vs #eta", 40, -2.5, 2.5);
  candidateEffVsPt_denom_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sEffVsPt_denom", "K^{0}_{S} Efficiency vs p_{T}", 70, 0., 20.);

  candidateFakeVsR_num_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sFakeVsR_num", "K^{0}_{S} Fake Rate vs #rho", 80, 0., 40.);
  candidateFakeVsEta_num_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sFakeVsEta_num", "K^{0}_{S} Fake Rate vs #eta", 40, -2.5, 2.5);
  candidateFakeVsPt_num_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sFakeVsPt_num", "K^{0}_{S} Fake Rate vs p_{T}", 70, 0., 20.);
  candidateTkFakeVsR_num_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sTkFakeVsR_num", "K^{0}_{S} Tracking Fake Rate vs #rho", 80, 0., 80.);
  candidateTkFakeVsEta_num_[QWV0Validator::KSHORT] =
      fKs.make<TH1D>("K0sTkFakeVsEta_num",
                     "K^{0}_{S} Tracking Fake Rate vs #eta", 40, -2.5, 2.5);
  candidateTkFakeVsPt_num_[QWV0Validator::KSHORT] =
      fKs.make<TH1D>("K0sTkFakeVsPt_num",
                     "K^{0}_{S} Tracking Fake Rate vs p_{T}", 70, 0., 20.);

  candidateFakeVsR_denom_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sFakeVsR_denom", "K^{0}_{S} Fake Rate vs #rho", 80, 0., 40.);
  candidateFakeVsEta_denom_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
      "K0sFakeVsEta_denom", "K^{0}_{S} Fake Rate vs #eta", 40, -2.5, 2.5);
  candidateFakeVsPt_denom_[QWV0Validator::KSHORT] = fKs.make<TH1D>(
  ...
  candidateEffVsR_num_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamEffVsR_num", "#Lambda^{0} Efficiency vs #rho", 80, 0., 40.);
  candidateEffVsEta_num_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamEffVsEta_num", "#Lambda^{0} Efficiency vs #eta", 40, -2.5, 2.5);
  candidateEffVsPt_num_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamEffVsPt_num", "#Lambda^{0} Efficiency vs p_{T}", 70, 0., 20.);

  candidateTkEffVsR_num_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamTkEffVsR_num", "#Lambda^{0} TrackingEfficiency vs #rho", 80, 0., 40.);
  candidateTkEffVsEta_num_[QWV0Validator::LAMBDA] =
      fLm.make<TH1D>("LamTkEffVsEta_num",
                     "#Lambda^{0} Tracking Efficiency vs #eta", 40, -2.5, 2.5);
  candidateTkEffVsPt_num_[QWV0Validator::LAMBDA] =
      fLm.make<TH1D>("LamTkEffVsPt_num",
                     "#Lambda^{0} Tracking Efficiency vs p_{T}", 70, 0., 20.);

  candidateEffVsR_denom_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamEffVsR_denom", "#Lambda^{0} Efficiency vs #rho", 80, 0., 40.);
  candidateEffVsEta_denom_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamEffVsEta_denom", "#Lambda^{0} Efficiency vs #eta", 40, -2.5, 2.5);
  candidateEffVsPt_denom_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamEffVsPt_denom", "#Lambda^{0} Efficiency vs p_{T}", 70, 0., 20.);

  candidateFakeVsR_num_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamFakeVsR_num", "#Lambda^{0} Fake Rate vs #rho", 80, 0., 40.);
  candidateFakeVsEta_num_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamFakeVsEta_num", "#Lambda^{0} Fake Rate vs #eta", 40, -2.5, 2.5);
  candidateFakeVsPt_num_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamFakeVsPt_num", "#Lambda^{0} Fake Rate vs p_{T}", 70, 0., 20.);
  candidateTkFakeVsR_num_[QWV0Validator::LAMBDA] =
      fLm.make<TH1D>("LamTkFakeVsR_num",
                     "#Lambda^{0} Tracking Fake Rate vs #rho", 80, 0., 40.);
  candidateTkFakeVsEta_num_[QWV0Validator::LAMBDA] =
      fLm.make<TH1D>("LamTkFakeVsEta_num",
                     "#Lambda^{0} Tracking Fake Rate vs #eta", 40, -2.5, 2.5);
  candidateTkFakeVsPt_num_[QWV0Validator::LAMBDA] =
      fLm.make<TH1D>("LamTkFakeVsPt_num",
                     "#Lambda^{0} Tracking Fake Rate vs p_{T}", 70, 0., 20.);

  candidateFakeVsR_denom_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamFakeVsR_denom", "#Lambda^{0} Fake Rate vs #rho", 80, 0., 40.);
  candidateFakeVsEta_denom_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
      "LamFakeVsEta_denom", "#Lambda^{0} Fake Rate vs #eta", 40, -2.5, 2.5);
  candidateFakeVsPt_denom_[QWV0Validator::LAMBDA] = fLm.make<TH1D>(
  ...
  candidateStatus_[QWV0Validator::LAMBDA] =
      fLm.make<TH1D>("ksCandStatus", "Fake type by cand status", 10, 0., 10.);
}
