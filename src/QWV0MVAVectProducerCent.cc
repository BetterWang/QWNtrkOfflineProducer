...
using namespace std;
class QWV0MVAVectProducerCent : public edm::EDProducer {
public:
	explicit QWV0MVAVectProducerCent(const edm::ParameterSet&);
	~QWV0MVAVectProducerCent();
...
QWV0MVAVectProducerCent::QWV0MVAVectProducerCent(const edm::ParameterSet& pset) :
	...
	rapMax_(pset.getUntrackedParameter<double>("rapMax",  2.5))
{
    ...
    reader->AddSpectator( "Cent",             &Cent_           );

    std::string xml = "TMVAClassification_BDT250_D4.LM_2.weights.xml";
    if ( V0Src_.instance() == "Lambda" ) {
        xml = "TMVAClassification_BDT250_D4.LM_";
    } else if ( V0Src_.instance() == "Kshort" ) {
        xml = "TMVAClassification_BDT250_D4.KS_";
    }
    reader->BookMVA( string("BDT1_")+V0Src_.instance(), xml+"1.weights.xml");
    reader->BookMVA( string("BDT2_")+V0Src_.instance(), xml+"2.weights.xml");
    reader->BookMVA( string("BDT3_")+V0Src_.instance(), xml+"3.weights.xml");
    reader->BookMVA( string("BDT4_")+V0Src_.instance(), xml+"4.weights.xml");
...
QWV0MVAVectProducerCent::~QWV0MVAVectProducerCent()
{
	return;
}

void QWV0MVAVectProducerCent::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	using namespace edm;
	...
	for ( auto v0 : (*V0s) ) {
		...
		if ( v0.numberOfDaughters() != 2 ) continue;
            ...
            double mva = 0.;
            if ( Cent_ >= 10. and Cent_ < 20. ) {
                mva = reader->EvaluateMVA(string("BDT1_")+V0Src_.instance());
            } else if ( Cent_ >= 20. and Cent_ < 60. ) {
                mva = reader->EvaluateMVA(string("BDT2_")+V0Src_.instance());
            } else if ( Cent_ >= 60. and Cent_ < 100. ) {
                mva = reader->EvaluateMVA(string("BDT3_")+V0Src_.instance());
            } else if ( Cent_ >= 100. and Cent_ < 160. ) {
                mva = reader->EvaluateMVA(string("BDT4_")+V0Src_.instance());
            }
...
DEFINE_FWK_MODULE(QWV0MVAVectProducerCent);
