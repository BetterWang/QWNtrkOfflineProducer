...
class QWDTreeMaker : public edm::stream::EDAnalyzer<> {
public:
	explicit QWDTreeMaker(const edm::ParameterSet&);
	~QWDTreeMaker() {};
private:
...
QWDTreeMaker::QWDTreeMaker(const edm::ParameterSet& pset) :
...
void
QWDTreeMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	using namespace edm;
	for ( auto tag : vTags_ ) {
...
DEFINE_FWK_MODULE(QWDTreeMaker);
