...
private:
	...
	virtual void beginRun(edm::Run&, const edm::EventSetup&) override;
	///
...
void QWZDCProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	//	std::cout << "i->id().zside() = " << i->id().zside() << " i->id().section() = " << i->id().section() << " i->id().channel() = " << i->id().channel() << std::endl;
		...
		if ( adc[i][3] > 126 ) {
			signal[i] = (reg_fC[i][4] + reg_fC[i][5] - 0.666 * ped) * hilo_ratio_[runid][i%9];
		} else {
...
{
	unordered_map<int, array<double, 9> >  const hilo = {
...
//		{286520, {0.0, 0.0, 12.211652358680652, 0.0, 0.0, 10.325458450355196, 11.988021568848408, 10.576683067576544, 0.0}}
	...
	for ( int i = 0; i < 18; i++ ) {
		weights_[i] = weights[i] / gain_[r.run()] / 2000. * 2.5617;
	}
